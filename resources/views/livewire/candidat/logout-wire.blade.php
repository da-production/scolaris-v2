<button type="button" wire:click="logout" class="bg-white px-3 py-0.5 rounded-lg shadow gap-2 cursor-pointer flex items-center w-full text-[#111418] text-sm font-medium leading-normal" role="menuitem">
    <x-icons.sign-out class="size-4 " />
    Déconnexion
</button>

{{-- 
<button type="button" wire:click="logout" class="flex w-full rounded-md px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 active:bg-blue-100 cursor-pointer  items-center justify-center gap-2" role="menuitem">
    <x-icons.sign-out class="size-4 " />
    Déconnexion
</button>
 --}}