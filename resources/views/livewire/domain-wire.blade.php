<x-layouts.option>
    <div class="flex w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="relative h-full flex-1  rounded-xl p-2 border border-neutral-200 dark:border-neutral-700">
            <div class="w-full h-full bg-white relative z-10 rounded-lg p-4 border">
                <div class="w-full">
                    <h3 class="text-lg font-semibold ml-3 text-slate-800">Gestion des <b>domains</b></h3>
                    <p class="text-slate-500 mb-5 ml-3"> modifiez et gérez les parametres des domains.</p>
                </div>
                    ...
                    <table class="w-full text-left table-auto min-w-max">
                                ...
                                <th class="p-4 border-b border-slate-300 bg-slate-50 w-11">
                                    <div class="w-full flex justify-end">
                                        <flux:modal.trigger name="add-domain-modal">
                                            <flux:button variant="primary" size="sm" x-data="" x-on:click.prevent="$dispatch('open-modal', 'add-domain-modal')">
                        ...
                        <tbody wire:sortable="updateOrder" wire:sortable.options="{ animation: 100 }">
                            @foreach ($domains as $domain)
                                <tr class="hover:bg-slate-50" wire:sortable.item="{{ $domain->id }}" wire:key="domain-{{ $domain->id }}">
                                    ...
                                    <td class="p-4 border-b border-slate-200">
                                        <div class="flex justify-end text-sm text-slate-800">
                                            ...
                                            <x-dropdown trigger="Options">
                                                <button wire:click="editDomain('{{$domain->id}}')"  class="cursor-pointer rounded-md block text-left w-full px-4 py-2 hover:bg-gray-100">{{ __('edit') }}</button>
                                                <button x-on:click="confirm('Are you sure u want to delete {{ $domain->name_fr }}') ? @this.call('delete','{{$domain->id}}') : null"  class="cursor-pointer rounded-md block text-left w-full px-4 py-2 text-red-500 hover:bg-red-100">{{ __('delete') }}</button>
                                            </x-dropdown>
    ...
    <flux:modal name="add-domain-modal" :show="$errors->isNotEmpty()" focusable class="max-w-xl w-full">
        <form wire:submit="save" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('Ajouter un domain') }}</flux:heading>

            </div>

            <flux:input wire:model="name_fr" :label="__('Lebelle FR')" type="text" />
            <flux:input wire:model="name_ar" :label="__('Lebelle AR')" type="text" />
            <flux:input wire:model="description" :label="__('Description')" type="textarea" />
