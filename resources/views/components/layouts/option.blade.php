<section class="w-full">
    ...
    <div class="flex items-start max-md:flex-col">
        <div class="mr-10 w-full pb-4 md:w-[220px]">
            <flux:navlist>
                <flux:navlist.item :href="route('administrateur.options.index')" wire:navigate >
                    <div class="flex gap-2 items-center">
                        <x-icons.users class="size-4" />
                        <span>{{ __('Scolaris') }}</span>
                    </div>
                </flux:navlist.item>
                <flux:navlist.item :href="route('administrateur.options.inscription')" wire:navigate>
                    <div class="flex gap-2 items-center">
                        <x-icons.roles class="size-4" />
                        <span>{{ __('Inscription') }}</span>
                    </div>
                </flux:navlist.item>
                <flux:navlist.item :href="route('administrateur.options.classifications')" wire:navigate>
                    <div class="flex gap-2 items-center">
                        <x-icons.roles class="size-4" />
                        <span>{{ __('Classifications') }}</span>
                    </div>
                </flux:navlist.item>
                <flux:navlist.item :href="route('administrateur.options.specialites')" wire:navigate>
                    <div class="flex gap-2 items-center">
                        <x-icons.roles class="size-4" />
                        <span>{{ __('Spécialité') }}</span>
                    </div>
                </flux:navlist.item>
                <flux:navlist.item :href="route('administrateur.options.exercices')" wire:navigate>
                    <div class="flex gap-2 items-center">
                        <x-icons.roles class="size-4" />
